...
for ray_index = 1:pn
    ...
    rayo(ray_index).th = atan2(rayo(ray_index).yc,rayo(ray_index).xc); %atan2 da el signo correcto 
end
%-----------------------------fin de rayos--------------------------------%
...
%     writeVideo(vidObj1, getframe(gca));
    ...
    % dos puntos
    H1r =      laguerregz(nu,mu,initialWaist,RayleighDistance,x,zDistance)....
        + 1i*pxlaguerregz(nu,mu,initialWaist,RayleighDistance,x,zDistance);
