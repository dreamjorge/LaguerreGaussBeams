...
mapgreen = AdvancedColormap('kgg',256,[0 100 255]/255);  %color del haz
...
Dz=zl;%*1.105*zl; % Tamaño de la ventana en z (distancia a la cual propagar)
Nz=2^9;          % Número de puntos en z
...
Xn=exp(1i*mu*TH).*pxlaguerregz(nu,mu,wo,zl,R,0);
...
sigmaLo=wo*sqrt((2*nu+mu+1));   %cintura de Laguerre en z=0
lo=sigmaLo/4;%sigmaLo/4;                   %tamaño de la obstrucción (radio de la obstruccion)
%traslado
xt=0;%sigmaLo/2.5;%.15*sigmaLo;
 yt=0;   %xt=0;
...
pn=8;        %numero de puntos que queremos muestrear alrededor del circulo
...
H1r=laguerregz(nu,mu,wo,zl,x,z(1))+1i*pxlaguerregz(nu,mu,wo,zl,x,z(1));
H2r=laguerregz(nu,mu,wo,zl,x,z(1))-1i*pxlaguerregz(nu,mu,wo,zl,x,z(1));
...
for jj=1:pn
    ...
    % para r=cte;
    H1z=laguerregz(nu,mu,wo,zl,ri,z)+1i*pxlaguerregz(nu,mu,wo,zl,ri,z);
    ...
    H2z=laguerregz(nu,mu,wo,zl,ri,z)-1i*pxlaguerregz(nu,mu,wo,zl,ri,z);
...
%     writeVideo(vidObj1, getframe(gca));
    ...
    % dos puntos
    H1r=laguerregz(nu,mu,wo,zl,x,z(ii))+1i*pxlaguerregz(nu,mu,wo,zl,x,z(ii));
    for jj=1:pn
        ...
        % para r=cte;
        H1z=laguerregz(nu,mu,wo,zl,rayo(jj).rH1,z)+1i*pxlaguerregz(nu,mu,wo,zl,rayo(jj).rH1,z);
        ...
        if  rayo(jj).rH2<0
            H2r=laguerregz(nu,mu,wo,zl,x,z(ii))+1i*pxlaguerregz(nu,mu,wo,zl,x,z(ii));
            H2z=laguerregz(nu,mu,wo,zl,rayo(jj).rH2,z)+1i*pxlaguerregz(nu,mu,wo,zl,rayo(jj).rH2,z);
